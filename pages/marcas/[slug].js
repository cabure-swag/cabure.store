...
function useBrand(slug){
  const [brand, setBrand] = useState(null); // null = cargando, {} = sin datos pero render
  useEffect(() => {
    let cancel = false;
    (async () => {
      if(!slug) return;
      try {
        ...
        if (cancel) return;
        if (error) {
          console.warn('brands select error', error);
          setBrand({}); // evita spinner infinito
        } else {
          setBrand(data || {});
        }
      } catch (e) {
        console.warn('brands select exception', e);
        if (!cancel) setBrand({});
      }
    })();
    return () => { cancel = true; };
...
function useBrandCovers(slug, brand){
  ...
  useEffect(() => {
    ...
    (async () => {
      if (!slug) return;
      // fallback inmediato por si algo falla
      const fallback = [brand?.cover_url || brand?.logo_url || '/logo.png'].filter(Boolean);

      // 1) Intentar brand_covers (si no hay tabla/RLS, se ignora)
      try {
        const { data, error } = await supabase
          .from('brand_covers')
          .select('url, position')
        ...
        console.warn('brand_covers select exception (ignorado):', e?.message || e);
      }

      // 2) Si no hay brand_covers, usar fallback
      if (!cancel) setCovers(fallback);
    })();
    return () => { cancel = true; };
  }, [slug, brand?.cover_url, brand?.logo_url]);
  return covers;
}
  ...
  useEffect(() => {
    ...
    supabase.from('categories').select('id,name').eq('brand_slug', slug).order('name')
      .then(({data, error}) => {
        if (cancel) return;
        if (error) {
          console.warn('categories select error', error);
          setCats([]);
        } else {
          setCats(data || []);
        }
      ...
      .catch(e => {
        console.warn('categories select exception', e);
        if (!cancel) setCats([]);
      });
    return () => { cancel = true; };
  }, [slug]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (cancel) return;
        if (e1) { console.warn('products error', e1); setItems([]); return; }
        ...
        console.warn('products select exception', e);
        if (!cancel) setItems([]);
      }
    })();
    return () => { cancel = true; };
...
export default function BrandPage(){
  ...
  // Estado “cargando” sólo si brand es null (fetch en curso)
  if (brand === null) {
  ...
  return (
    <main>
      {/* HERO (portada rotatoria, siempre tiene al menos un fallback) */}
      <div className="hero">
        <RotatingCover
          images={covers}
