...
export default function Checkout(){
  ...
  const hasDomicilio = Number.isFinite(brand?.ship_domicilio);
  const hasSucursal = Number.isFinite(brand?.ship_sucursal);
  ...
  const mpPercent = Number.isFinite(brand?.mp_fee) ? brand.mp_fee : null;
  const mpFee = pay==='mp' ? Math.round(subtotal * ((mpPercent||10)/100)) : 0;
  const totalFinal = subtotal + shipCost + mpFee;

  async function confirm(e){
    e.preventDefault();
    setError('');
    if (!shipping) { setError('Elegí un método de envío.'); return; }
    setBusy(true);
    try {
      const { data: sess } = await supabase.auth.getSession();
      const user = sess?.session?.user;
      if (!user) { setError('Iniciá sesión con Google.'); setBusy(false); return; }
      if (pay==='mp' && !brand.mp_access_token){ setError('MP deshabilitado: admin debe configurar token en /admin.'); setBusy(false); return; }

      const orderPayload = {
        user_id: user.id, brand_slug: brandSlug, shipping, pay,
        mp_fee: mpPercent || 10, ship_cost: shipCost, subtotal, total: totalFinal
      };
      const { data: order, error: e1 } = await supabase.from('orders').insert(orderPayload).select('*').single();
      if (e1) throw e1;
      const itemsPayload = cartLines.map(l => ({ order_id: order.id, product_id: l.id, name: l.name, price: l.price, qty: l.qty }));
      ...
      if (pay==='mp'){
        const res = await fetch('/api/mp/create-preference', {
          ...
          body: JSON.stringify({ orderId: order.id, brandSlug, items: cartLines.map(l=> ({ title:l.name, quantity:l.qty, unit_price:l.price })) })
        });
        ...
        <form className="card" onSubmit={confirm}>
          <div className="mb">
            ...
            <select value={shipping} onChange={e=>setShipping(e.target.value)}>
              ...
              {hasDomicilio && <option value="domicilio">Correo Argentino a domicilio {freeFrom>0 && subtotal>=freeFrom ? '(GRATIS)' : ''}</option>}
              {hasSucursal && <option value="sucursal">Correo Argentino a sucursal {freeFrom>0 && subtotal>=freeFrom ? '(GRATIS)' : ''}</option>}
            </select>
          </div>
          <div className="mb">
            <label>Pago</label>
            <select value={pay} onChange={e=>setPay(e.target.value)}>
              <option value="mp">Mercado Pago (+{mpPercent || 10}%)</option>
              <option value="transferencia">Transferencia</option>
            </select>
          </div>
          {error && <p className="small" style={{color:'#f87171'}}>{error}</p>}
          <button className="btn" disabled={cartLines.length===0 || (!hasDomicilio && !hasSucursal) || busy}>{busy?'Confirmando...':'Confirmar pedido'}</button>
        </form>
        <div className="card">
          ...
          <div className="row"><span>Envío</span><span>{money(shipCost)}</span></div>
          {pay==='mp' && <div className="row"><span>Recargo MP</span><span>{money(mpFee)}</span></div>}
          <div className="row"><strong>Total</strong><strong>{money(totalFinal)}</strong></div>
        </div>
      </div>
    </main>
  );
