// pages/index.js
// Mismo layout/estilos. Sólo agrega rotación de cover_photos y usa avatar_url||logo_url.
...
// Componente interno ultra-minimalista que rota el src cada 10s si hay varias imágenes.
// No cambia estilos: el padre define tamaño y clases.
function RotatingCover({ covers = [], alt = '', style }) {
  const pics = useMemo(() => (Array.isArray(covers) ? covers.filter(Boolean) : []), [covers]);
  ...
  const current = pics.length ? pics[idx] : null;
...
export default function HomePage() {
  ...
  return (
    <>
      ...
      {/* Banda de logos: ahora usa avatar_url||logo_url por dentro */}
      <section>
        <LogoTickerSeamless brands={brands} pxPerSec={40} />
      </section>

      {/* Grid de marcas (mantené tus clases/estilos existentes alrededor de este markup) */}
      <main className="container" style={{ paddingTop: 18 }}>
        <div className="grid" style={{ gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 18 }}>
          {brands.map((b) => {
            // decisión de fuente para cover: si hay cover_photos, rotamos; si no, usamos cover_url || logo/avatar
            const hasCovers = Array.isArray(b.cover_photos) && b.cover_photos.length > 0;
            const staticCover = b.cover_url || b.logo_url || b.avatar_url || '/logo.png';
            const avatar = b.avatar_url || b.logo_url || '/logo.png';
              ...
              <Link key={b.slug} href={`/marcas/${b.slug}`} className="brandCard" style={{ textDecoration: 'none' }}>
                <div className="card" style={{ padding: 0, overflow: 'hidden' }}>
                  <div className="cover" style={{ position: 'relative', height: 220, background: '#0e0f16' }}>
                    {hasCovers ? (
                      <RotatingCover covers={b.cover_photos} alt={b.name} style={{ filter: 'brightness(.85)' }} />
                    ) : (
                      <img
                        src={staticCover}
                        alt={b.name}
                    ...
                    {/* Overlay inferior con logo + nombre + IG (mantén tus estilos/clases) */}
                    <div
                      className="overlay"
                      style={{
                        position: 'absolute',
                      ...
                      {b.instagram && (
                        ...
                        >
                          <span className="small">
                            @{b.instagram.replace(/^https?:\/\/(www\.)?instagram\.com\//, '').replace(/\/$/, '')}
                          </span>
