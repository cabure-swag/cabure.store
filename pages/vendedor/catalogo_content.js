...
export default function VCatalogo(){
  ...
  useEffect(()=>{ if(!session) return; let on=true; (async()=>{
    ...
    if(!on) return;
    setBrands(b); if(!slug && b?.[0]?.slug) setSlug(b[0].slug);
  ...
  async function onMainImage(urls){ if(!edit || !urls?.length) return; await updateProduct(edit.id, { image_url: urls[0] }); }
  ...
  return (
    <section>
      ...
      <div className="card">
        ...
        <select value={slug} onChange={e=>setSlug(e.target.value)}>{brands.map(b=>(<option key={b.slug} value={b.slug}>{b.name}</option>))}</select>
      </div>
      <div className="card">
        ...
        <div className="list">
          {items.map(p=>(
            <div key={p.id} className="row item">
              <div className="left">
                <img src={p.image_url || '/logo.png'} className="thumb" alt="" onClick={()=>setEdit(p)}/>
                <div>
          ...
          <div className="box" onClick={e=>e.stopPropagation()}>
            ...
            <div className="row gap">
              ...
              <div style={{flex:1}}>
                ...
                <div className="lbl">Galería (recomendado 1:1)</div>
