...
export default function VPerfil(){
  ...
  useEffect(()=>{ if(!session) return; let on = true; (async ()=>{
    ...
    if(!on) return;
    setList(b); if(!slug && b?.[0]?.slug) setSlug(b[0].slug);
  ...
  return (
    <section>
        ...
        <div className="card">
          <div className="row gap">
            ...
            <div style={{flex:1}}>
              ...
              <textarea rows={3} defaultValue={brand.description||''} onBlur={(e)=>upd('description', e.target.value)}/>
              <div className="row gap">
                <div style={{flex:1}}>
          ...
          <div className="thumbs">{(brand.cover_urls||[]).map((u,i)=>(<img key={i} src={u} alt=""/>))}</div>
      ...
      <style jsx>{`
        ...
        .row{ display:flex; align-items:flex-start; } .gap{ gap:12px; }
        .logo{ width:100%; height:140px; object-fit:cover; border:1px solid var(--line); border-radius:12px; margin-bottom:8px; }
