    ...
    <div ref={wrapRef} className="cab-wrap" aria-label="Banda de logos">
      <div ref={t1Ref} className="cab-track">
        {items.map((b, i) => {
          ...
          // DIF 1: si hay slug, Link; si no, div. Clase .cab-slot intacta
          return b?.slug ? (
      ...
      <div ref={t2Ref} className="cab-track">
        {items.map((b, i) => {
          ...
          // DIF 1 (track 2): igual que arriba
          return b?.slug ? (
            <Link key={key} href={`/marcas/${b.slug}`} className="cab-slot" aria-label={b?.name || b.slug}>
              {content}
            </Link>
      ...
      <style jsx>{`
        ...
        .cab-slot {
          ...
          overflow: hidden;      /* asegura círculo perfecto con recorte */
        }
        .cab-img {
          display: block;
